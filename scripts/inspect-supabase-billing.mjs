...
try {
  ...
  const [tables, columns, constraints, policies, rls] = await Promise.all([
  ...
  console.log(JSON.stringify({ tables: tables.rows, columns: columns.rows, constraints: constraints.rows, policies: policies.rows, rls: rls.rows }, null, 2));
} finally {
  await client.end();
}
