...
export function ProfileHero({ portfolio, compact = false, className }: { portfolio: TemplatePortfolio; compact?: boolean; className?: string }) {
  ...
  return <header className={cn("flex items-start justify-between gap-5", className)}><div className="flex min-w-0 items-center gap-4"><div className={cn("grid shrink-0 place-items-center overflow-hidden bg-[var(--p-soft)] font-bold text-[var(--p-accent)]", compact ? "size-12 rounded-2xl" : "size-20 rounded-[1.4rem] text-2xl")}>{portfolio.avatarUrl ? <OptimizedImage src={portfolio.avatarUrl} alt={`Portrait of ${portfolio.title}`} className="size-full" priority /> : initial}</div><div className="min-w-0"><h1 className={cn("text-balance font-semibold tracking-[-0.055em]", compact ? "text-2xl sm:text-3xl" : "text-4xl sm:text-5xl")}>{portfolio.title}</h1><p className={cn("mt-2 max-w-2xl whitespace-pre-line leading-7 text-[var(--p-muted)]", compact ? "text-sm" : "text-base sm:text-lg")}>{portfolio.bio}</p></div></div>{portfolio.logoUrl ? <OptimizedImage src={portfolio.logoUrl} alt="Portfolio logo" className="max-h-10 max-w-24" priority /> : null}</header>;
}

export function SocialNav({ portfolio }: { portfolio: TemplatePortfolio }) {
  if (!portfolio.socialLinks.length) return null;
  return <nav aria-label="Social profiles" className="flex flex-wrap gap-2">{portfolio.socialLinks.map((link) => { const Icon = socialIcons[link.platform]; return <a key={link.id} href={link.url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 rounded-full border border-[var(--p-border)] bg-[var(--p-surface)] px-3 py-1.5 text-xs font-semibold text-[var(--p-text)] transition-colors duration-200 hover:-translate-y-0.5 hover:bg-[var(--p-soft)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--p-accent)] focus-visible:ring-offset-2"><Icon className="size-3.5" />{labels[link.platform]}</a>; })}</nav>;
}

export function SectionHeading({ eyebrow, title, children }: { eyebrow?: string; title: string; children?: ReactNode }) {
  return <div className="mb-6"><p className="text-xs font-bold uppercase tracking-[0.16em] text-[var(--p-accent)]">{eyebrow || "Selected work"}</p><h2 className="mt-2 text-2xl font-semibold tracking-[-0.045em] sm:text-3xl">{title}</h2>{children}</div>;
}
...
export function ContactForm({ portfolio, title = "Let’s work together" }: { portfolio: TemplatePortfolio; title?: string }) {
  ...
  return <section aria-labelledby="contact-title" className="print:break-inside-avoid"><div className="rounded-[1.65rem] bg-[var(--p-soft)] p-6 sm:p-8 print:rounded-none print:border print:border-black/20 print:bg-white"><p className="text-xs font-bold uppercase tracking-[0.16em] text-[var(--p-accent)] print:text-black">Contact</p><h2 id="contact-title" className="mt-2 text-3xl font-semibold tracking-[-0.05em]">{title}</h2><p className="mt-2 max-w-xl text-sm leading-6 text-[var(--p-muted)] print:text-black/70">Have a thoughtful project in mind? Send a short note and I’ll reply as soon as I can.</p><p className="mt-4 hidden text-sm font-semibold print:block">Email: {email}</p><form action={`mailto:${email}`} method="post" encType="text/plain" onSubmit={submit} className="mt-6 grid gap-3 sm:grid-cols-2 print:hidden"><div><label htmlFor="contact-name" className="sr-only">Your name</label><input id="contact-name" name="name" required placeholder="Your name" className="h-11 w-full rounded-xl border border-[var(--p-border)] bg-[var(--p-surface)] px-3 text-sm text-[var(--p-text)] placeholder:text-[var(--p-muted)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--p-accent)]" /></div><div><label htmlFor="contact-email" className="sr-only">Your email</label><input id="contact-email" name="email" type="email" required placeholder="Your email" className="h-11 w-full rounded-xl border border-[var(--p-border)] bg-[var(--p-surface)] px-3 text-sm text-[var(--p-text)] placeholder:text-[var(--p-muted)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--p-accent)]" /></div><div className="sm:col-span-2"><label htmlFor="contact-message" className="sr-only">Your message</label><textarea id="contact-message" name="message" required rows={4} placeholder="A little about your project" className="w-full rounded-xl border border-[var(--p-border)] bg-[var(--p-surface)] px-3 py-3 text-sm text-[var(--p-text)] placeholder:text-[var(--p-muted)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--p-accent)]" /></div><button type="submit" className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-[var(--p-accent)] px-5 text-sm font-semibold text-white transition-transform duration-200 hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--p-accent)] focus-visible:ring-offset-2 sm:justify-self-start"><Mail className="size-4" />Send an email</button></form></div></section>;
}
