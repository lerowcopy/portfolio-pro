...
export function PortfolioPreview({ portfolio, className, showFrame = true }: { portfolio: PreviewPortfolio; className?: string; showFrame?: boolean }) {
  ...
  return <div className={className}>{showFrame ? <div className="mb-3 flex items-center justify-between gap-3 print:hidden"><p className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-[0.13em] text-slate-500 dark:text-slate-400"><span className="size-1.5 rounded-full bg-emerald-500" />{t("preview.live")}</p><div className="flex items-center gap-2"><span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-600 dark:bg-white/10 dark:text-slate-300">{labels[portfolio.template]}</span><LanguageToggle /></div></div> : null}<div style={style} className={cn("bg-[var(--p-bg)] text-[var(--p-text)] [font-family:var(--p-font)] print:bg-white print:text-black", showFrame && "overflow-hidden rounded-[1.5rem] border border-slate-200 shadow-[0_18px_45px_-28px_rgba(15,23,42,0.35)] dark:border-white/10 print:rounded-none print:border-0 print:shadow-none", !showFrame && "min-h-svh")}><AnimatePresence mode="wait" initial={false}><motion.div key={portfolio.template} initial={reduceMotion ? { opacity: 0 } : { opacity: 0, y: 10, scale: 0.99 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={reduceMotion ? { opacity: 0 } : { opacity: 0, y: -7, scale: 0.99 }} transition={{ duration: 0.22, ease: [0.22, 1, 0.36, 1] }}><TemplateContent portfolio={portfolio} /></motion.div></AnimatePresence></div></div>;
}
