...
export function PortfolioCard({ portfolio, onDelete }: PortfolioCardProps) {
  ...
  const date = new Date(portfolio.updatedAt).toLocaleDateString(locale === "ru" ? "ru-RU" : "en", { month: "short", day: "numeric" });
  ...
  return (
    <article className="group overflow-hidden rounded-[1.45rem] border border-slate-200/90 bg-white shadow-[0_12px_32px_-22px_rgba(15,23,42,0.28)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_20px_45px_-25px_rgba(15,23,42,0.38)] dark:border-white/10 dark:bg-slate-900/80 dark:shadow-black/20">
      <div className={`relative h-32 overflow-hidden bg-gradient-to-br ${swatches[portfolio.colorScheme]}`}>
        ...
        <div className="absolute right-4 top-4"><Badge className={portfolio.isPublished ? "border-0 bg-white/90 text-slate-900 hover:bg-white" : "border-0 bg-slate-950/35 text-white hover:bg-slate-950/35"}>{portfolio.isPublished ? t("card.published") : t("card.draft")}</Badge></div>
      </div>
      <div className="relative p-5 pt-0">
        <div className="-mt-8 flex items-end justify-between gap-3">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild><Button variant="ghost" size="icon" className="mb-1 rounded-full text-slate-500 hover:bg-slate-100 hover:text-slate-950 dark:hover:bg-white/10 dark:hover:text-white" aria-label={t("card.moreActions", { title: portfolio.title })}><MoreHorizontal className="size-5" /></Button></DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="w-44 rounded-xl border-slate-200 p-1.5 dark:border-white/10">
              <DropdownMenuItem onClick={() => navigator.clipboard.writeText(`${window.location.origin}/${portfolio.slug}`)} className="rounded-lg"><Copy className="mr-2 size-4" />{t("card.copyUrl")}</DropdownMenuItem>
              <DropdownMenuSeparator />
              <DropdownMenuItem onClick={() => onDelete(portfolio.id)} className="rounded-lg text-destructive focus:text-destructive"><Trash2 className="mr-2 size-4" />{t("card.delete")}</DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
        <div className="mt-4">
          ...
          <div className="mt-5 flex items-center justify-between"><p className="text-xs text-slate-400 dark:text-slate-500">{t("card.updated", { date })}</p>{portfolio.isPublished ? <a href={`/${portfolio.slug}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 rounded-md text-xs font-semibold text-violet-700 transition-colors hover:text-violet-900 focus-visible:ring-2 focus-visible:ring-violet-500 dark:text-violet-300"><span>{t("card.view")}</span><ArrowUpRight className="size-3.5" /></a> : <span className="text-xs font-medium text-slate-400">{t("card.private")}</span>}</div>
        </div>
      </div>
    </article>
  );
