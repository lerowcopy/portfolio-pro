...
export default function ProjectFormPage() {
  ...
  const rawPortfolioId = createParams?.id || editParams?.id || "";
  const rawProjectId = editParams?.projectId;
  const portfolioId = isExternalRuntime ? rawPortfolioId : Number(rawPortfolioId);
  const projectId = rawProjectId ? (isExternalRuntime ? rawProjectId : Number(rawProjectId)) : undefined;
  const { isAuthenticated } = useAuth();
  const [, setLocation] = useLocation();
  const projectQuery = trpc.projects.get.useQuery({ portfolioId: portfolioId as never, projectId: (projectId ?? 0) as never }, { enabled: isAuthenticated && Boolean(projectId) && (isExternalRuntime ? Boolean(portfolioId) : Number(portfolioId) > 0) });
  ...
  const externalProject = projectQuery.data as (typeof projectQuery.data & { imageStoragePaths?: string[] }) | undefined;
  const displayImages = externalProject?.images ?? [];
  const storedImages = isExternalRuntime ? externalProject?.imageStoragePaths ?? [] : displayImages;
  const initial = externalProject ? { title: externalProject.title, description: externalProject.description, images: storedImages, tags: externalProject.tags, projectUrl: externalProject.projectUrl, startDate: externalProject.startDate, endDate: externalProject.endDate } : emptyProject;
  const initialPreviewUrls = Object.fromEntries(storedImages.map((path, index) => [path, displayImages[index] ?? path]));
  return <ProjectForm portfolioId={portfolioId} projectId={projectId} initial={initial} initialPreviewUrls={initialPreviewUrls} onDone={() => setLocation(`/dashboard/portfolios/${portfolioId}/projects`)} />;
}

function ProjectForm({ portfolioId, projectId, initial, initialPreviewUrls, onDone }: { portfolioId: string | number; projectId?: string | number; initial: ProjectInput; initialPreviewUrls: Record<string, string>; onDone: () => void }) {
  ...
  async function uploadImage(file: File) {
    ...
    try {
      ...
      const result = await upload.mutateAsync({ portfolioId: portfolioId as never, mimeType: file.type as "image/jpeg" | "image/png" | "image/webp", base64 }) as unknown as { url: string; storagePath?: string };
      if (isExternalRuntime && !result.storagePath) throw new Error("Хранилище не вернуло защищённый путь изображения.");
      const storedPath = result.storagePath ?? result.url;
      setPreviewUrls((current) => ({ ...current, [storedPath]: result.url }));
      return storedPath;
    } finally { setUploading(false); }
  }
  async function submit(values: ProjectInput) { if (uploading) return; if (projectId) await update.mutateAsync({ portfolioId: portfolioId as never, projectId: projectId as never, values }); else await create.mutateAsync({ portfolioId: portfolioId as never, values }); await utils.projects.list.invalidate(); await utils.portfolios.get.invalidate({ id: portfolioId as never }); onDone(); }
