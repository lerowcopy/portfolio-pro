...
function PortfolioEditor({ portfolioId, initialValues, initialStoragePaths }: { portfolioId: string | number; initialValues: PortfolioInput; initialStoragePaths: { logo: string; avatar: string } }) {
  ...
  useEffect(() => { const beforeUnload = (event: BeforeUnloadEvent) => { if (!form.formState.isDirty) return; event.preventDefault(); event.returnValue = ""; }; window.addEventListener("beforeunload", beforeUnload); return () => window.removeEventListener("beforeunload", beforeUnload); }, [form.formState.isDirty]);
